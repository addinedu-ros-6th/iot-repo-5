...
class Motor: 
    ...
    def read_ino(self): 
        if self.py_serial.readable(): 
            ...
            # self.deviceStatus = int.from_bytes(response[0], 'little') 
            # self.ino_time = int.from_bytes(response[1:5], 'little')
            # self.position_x = int.from_bytes(response[5], 'little') 
            # self.position_y = int.from_bytes(response[6], 'little') 
            # self.direction_x = int.from_bytes(response[7], 'little') 
            # self.direction_y = int.from_bytes(response[8], 'little') 
            # self.motorStatus = int.from_bytes(response[9], 'little') 
            # self.flameStatus = int.from_bytes(response[10], 'little') 
            # self.waterStatus = int.from_bytes(response[11], 'little') 
    ...
    def send_cmd(self, center_x, center_y, manual_cmd=None): 
        ...
        print("cmd:", cmd[:-1], " state:", self.state) 
